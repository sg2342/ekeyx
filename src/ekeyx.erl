...
-export([generate_shares/3, recover_secret/1,
	 generate_shares/4, recover_secret/2]).


-spec generate_shares(K :: pos_integer(), N :: pos_integer(),
...
encode(Shares, binary) -> Shares;
encode(Shares, base16) ->
    lists:map(fun string:lowercase/1,
	      lists:map(fun binary:encode_hex/1, Shares));
encode(Shares, base64) -> lists:map(fun base64:encode/1, Shares).
